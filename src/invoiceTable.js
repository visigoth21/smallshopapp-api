...
var lineItem = [];
var lineChoices = [];
sessionStorage.setItem('UserToken', '6ef3c14ae45b367ff31790956b4d176e');
...
// ----------------------------------------------------------------------------
// https://www.youtube.com/watch?v=Ju5FGcyifEA     Authentication



// ----------------------------------------------------------------------------
var inputValueBox = document.getElementById('inputValue');
//-----------------------------------------------------------------------------
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var miscInput1 = document.getElementById('misc-quantity-input');
  var miscInput2 = document.getElementById('misc-upc-input');
  var miscInput3 = document.getElementById('misc-description-input');
  var miscInput4 = document.getElementById('misc-list-input');
  var miscInput5 = document.getElementById('misc-part-input');
  ...
  container.addEventListener('keydown', function(event) {
    ...
    if (target.tagName === 'INPUT' && event.keyCode === 13) {
      ...
      } else if (target === miscInput1) {
        document.getElementById("misc-add-btn").click();
      }else if (target === miscInput2) {
        document.getElementById("misc-add-btn").click();
      }else if (target === miscInput3) {
        document.getElementById("misc-add-btn").click();
      }else if (target === miscInput4) {
        document.getElementById("misc-add-btn").click();
      }else if (target === miscInput5) {
        document.getElementById("misc-add-btn").click();
      } else {
        $("#save-edit-btn").click();
      }
...
function printInvoice() {
  ...
  window.open(baseUrl + "/invoicePrint.php?PrintObj=" + JSON.stringify(items), "_blank");
 }
...
// Add item--------------------------------------------------------------------
$("#misc-add-btn").on("click", function () {
  var line = items.length + 1;
  var quantity = $("#misc-quantity-input").val();
  var id = "0";
  var part_number = $("#misc-part-input").val();
  var upc = $("#misc-upc-input").val();
  var description = $("#misc-description-input").val();
  var list = parseFloat($("#misc-list-input").val());
  var total = parseFloat($("#misc-quantity-input").val() * $("#misc-list-input").val());
  var taxable = 1;

  if (!$("#misc-taxable-input").is(':checked')) {
   var taxable = 0;
  }
...
$("#item-table").on("click", ".edit-btn", function () {
  ...
  var TDPartID = row.find("td:eq(10)").text();
  ...
  row.find("td:eq(5)").html(`<input type="number" id="list-row-input" onkeypress="return event.charCode >= 48" min="1" value=` + row.find("td:eq(5)").text() + `>`);
  row.find("td:eq(6)").html(`<input type="number" id="quantity-row-input" onkeypress="return event.charCode >= 48" min="1" value=` + row.find("td:eq(6)").text() + `>`);
...
//-----------------------------------------------------------------------------
// Function to update quantity based on id value
function updateTaxableByLineId(sLine, sValue) {
  // Loop through the array to find the item with matching id
  for (var i = 0; i < items.length; i++) {
    if (items[i].line === sLine) {
      // Update the quantity value
      items[i].taxable = sValue;
      renderTable();
      return true;
    }
  }
...
// Clear input widget
function clearInput() {
  $("#misc-quantity-input").val(0);
  $("#misc-part-input").val("");
  $("#misc-upc-input").val("");
  $("#misc-description-input").val("");
  $("#misc-list-input").val("0.00");
  $("#misc-total-input").val("0.00");
  $("#misc-id-input").val(0);
}
//-----------------------------------------------------------------------------
// Checkbox
function taxableCheckbox(sLine, sValue) {
  if (sValue == 1) { 
    return '<input type="checkbox" id="taxable-input" value="1" onclick="updateTaxableByLineId(' + sLine + ', 0)" checked></input>'; 
  } else {
    return '<input type="checkbox" id="taxable-input" value="0" onclick="updateTaxableByLineId(' + sLine + ', 1)" ></input>'; } 
}
//-----------------------------------------------------------------------------
...
function getItemFromSearch() {
  ...
  if (!inputValue == null || !inputValue == "") {
    $.ajax({
      ...
      type: "GET",
      dataType: "json",      
      headers: {"Cache-Control": "no-cache", "Authorization": "Bearer " + userToken,},
      success: function (data) {
        if (Number(data.data.length) > Number(1)) {
          setChoiceData(data.data);
        } else {
          $("#errorValue").text("No Items found");
      ...
      error: function (xhr, status, error) {
        ...
        $("#errorValue").text("No Items found for '" + inputValue + "'");
      },
    });
  }
  $("#inputValue").val("");
...
function renderTable() {
  ...
  $.each(items, function (index, item) {
    ...
    $("<td>").html(taxableCheckbox(item.line, item.taxable)).appendTo(row);    
